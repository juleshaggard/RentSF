...
export async function sendNewListingsDigest(listings: NormalizedListing[]) {
  ...
  const subject = `${listings.length} new SF ${listings.length === 1 ? "rental" : "rentals"}`;
      ...
      (listing) => `
        <tr>
          <td style="padding:18px 0;border-bottom:1px solid #ece7e0;">
            ...
            <div style="margin-top:8px;color:#2b2826;">${listing.rent ? `$${listing.rent.toLocaleString()}/mo` : "Rent not listed"} · ${bedroomLabel(listing.bedrooms)} · ${listing.bathrooms ?? "?"} bath</div>
  ...
  return `
    <div style="background:#fbfaf7;padding:28px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
      <div style="max-width:680px;margin:0 auto;background:#fffdf9;border:1px solid #ece7e0;border-radius:18px;padding:28px;">
        <h1 style="margin:0 0 8px;color:#2b2826;font-size:26px;">New SF rentals</h1>
  ...
  return listings
    .map((listing) => {
      return [
        listing.title,
        listing.address,
        `${listing.rent ? `$${listing.rent.toLocaleString()}/mo` : "Rent not listed"} · ${bedroomLabel(listing.bedrooms)}`,
        listing.url
      ].join("\n");
    })
    .join("\n\n");
...
function bedroomLabel(value: number | null) {
  if (value === null) return "beds not listed";
  if (value === 0) return "studio";
  if (value === 1) return "1 bed";
  return `${Number.isInteger(value) ? value : value.toFixed(1)} beds`;
}
